...
const LatestPost = () => {
  ...
  return (
    <div className=" w-full lg:w-4/6  mx-auto  flex  flex-col lg:px-6 justify-around gap-10 py-6">
      ...
      {auth ? <Button url={"/admin/updatePost"}/> : ''}
      </div>
      ...
      <div className="w-full  flex flex-wrap gap-4 justify-around">
        {card.map((item, idx) => {
          return (
            <div
              key={idx}
              className="relative w-80 lg:w-72 rounded-lg border border-gray-300  flex flex-col items-center justify-center text-start relative overflow-hidden shadow-lg"
            >
              {auth ? <EditDelete className={"absolute top-4 -right-24"}/> : ''}
